...
export async function sendErrorEmail(error: any) {
  ...
  const mailOptions = {
    ...
    html: `
      ...
      <code>${process.env.OJAS_URL || 'https://ojas.gujarat.gov.in/AdvtList.aspx?type=lCxUjNjnTp8='}</code>
      <hr>
      ...
      <p>You can update the <code>OJAS_URL</code> in GitHub Secrets if the website URL has changed.</p>
    `,
  };
